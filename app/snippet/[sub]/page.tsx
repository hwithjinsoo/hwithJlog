...
export default async function SnippetSubPage({ params }: Props) {
  const { sub } = await params;
  const posts = getAllPosts().filter(
    (post) => post.category === "snippet" && post.subcategory === sub
  );

  return (
    <div className="max-w-4xl mx-auto px-8 pt-12">
      <h1 className="text-3xl font-bold mb-2">Snippet - {sub}</h1>
