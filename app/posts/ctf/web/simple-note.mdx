...
전체 코드를 보면 이 웹사이트에는 "쓰기, 수정, 삭제, 백업" 기능이 존재한다.  
그 중 backup_notes 함수에 취약점이 존재한다.
...
def post_backup_notes():
 ...
 backup_notes 페이지에서 post 요청을 할 때를 봐보자.  
 백업 페이지에 접근하려하면 우선 백업 할 note가 있는지 확인하고 없다면 404 error를 띄운다.  
 후에 timestamp 함수로 쿠키를 생성하고 해당 값을 인자로서 backup_notes() 함수에 보낸다.
...
문제는 backup_notes 함수가 실행 될 때다.  
해당 코드를 보면 subcategory.Popen으로 shell 명령어를 실행 시키고 있는데 옵션을 보면 **shell=True**로 되어 있다.  
즉, 쉘 코드를 입력하면 실행이 된다는 얘기다.
...
값을 아래 이미지처럼 조작한 뒤 보내면 쉘을 서버에서 동작하게 할 수 있다.  
앞에 숫자는 timestamp 값으로 아무 값이나 넣어도 무관하며, curl을 사용해서  
서버로부터 받는 데이터를 어디로 보낼지 지정한다(해당 url은 드림핵 툴즈에서 발급 받을 수 있다)  
